...
import { TContactFormData } from '@/types/requestsTypes';
...
type Win = Window & {
  lenis?: {
    scrollTo?: (
      to: number,
      opts?: { duration?: number; immediate?: boolean; lock?: boolean },
    ) => void;
  };
  gsap?: {
    to: (
      target: Window,
      vars: { scrollTo: number; duration: number; ease?: string },
    ) => void;
  };
};

const smoothCenterScroll = (
  el: HTMLElement,
  opts?: { offset?: number; durationMs?: number },
) => {
  const offset = opts?.offset ?? 0;
  const durationMs = opts?.durationMs ?? 700;
  const rect = el.getBoundingClientRect();
  const targetY =
    window.scrollY + rect.top - (window.innerHeight - rect.height) / 2 + offset;
  const w = window as Win;
  if (w.lenis?.scrollTo) {
    w.lenis.scrollTo(targetY, {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const focusable =
        (anchor.matches('[name]')
          ? anchor
          : anchor.querySelector<HTMLElement>('[name]')) || anchor;
      window.setTimeout(
        () => focusable.focus({ preventScroll: true }),
        Math.max(200, Math.min(usedMs - 100, 800)),
      );
    } finally {
      afterRun();
    }
...
const ContactForm: React.FC = () => {
  const { mutate, isPending, isSuccess } = useSubmitContactForm(); // isError, data, error
  ...
  const submitFormHandler = (data: TContactFormData) => {
    mutate(data);
  };
            ...
            <div className={styles.formBody}>
              <div className={styles.formGroup}>
                ...
                <div className={styles.formGroupBody}>
                  ...
                  <div
                    className={styles.formGroupBodyRow}
