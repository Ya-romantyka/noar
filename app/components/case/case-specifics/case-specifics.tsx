...
const CaseSpecifics: React.FC<CaseSpecificsProps> = ({ fonts, colors }) => {
    ...
    const defaultSample = "“AcroStudio is a modern gym for children and their parents”";
            ...
            <Container>
                <div className={styles.header}>
                    <div className={styles.headerTop}>
                        <span className={clsx(styles.label, ["section-label section-label--white"])}>Specifics</span>
                    ...
                    <ul className={styles.list}>
                        {fonts.map((font, i) => (
                            <li key={i}>
                                <div className={styles.headerBottom} style={{ fontFamily: ff(font)}}>
                                    <div className={styles.col}>
                                        <h3 className={styles.subtitle} >
                                            {font.name}
                                        </h3>
                                        {font.author && <p className={styles.text}>by {font.author}</p>}
                                    </div>
                                    ...
                                    <div className={styles.col}>
                                        ...
                                        <ul className={styles.fonts} >
                                            {(font.list?.length ? font.list : defaultRoles).map((item, i) => (
